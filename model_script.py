...
def get_wiki():
    V = 20000
    files = glob.glob('./enwiki-preprocessed/*.txt')
    all_word_counts = {}  # have to count the words to know which are the freq/infreq words
    for f in files:
        for line in open(f):
            # check if sentence starts with special characters (usually title/metadata)
            if line and line[0] not in '[*-|=\{\}':
                # pre-process the sentence into tokens
    ...
    V = min(V, len(all_word_counts))  # get vocabs
    # sort words according to their counts in desc order
    all_word_counts = sorted(all_word_counts.items(),
                             key=lambda x: x[1], reverse=True)

    top_words = [w for w, count in all_word_counts[:V-1]] + ['<UNK>']
    # map unique words to an integer from 0 to 20000
    word2idx = {w: i for i, w in enumerate(top_words)}
    unk = word2idx['<UNK>']

    sents = []  # map sentence to a list of tokens
    for f in files:
        for line in open(f):
            if line and line[0] not in '[*-|=\{\}':
                s = remove_punctuation(line).lower().split()
    ...
    for epoch in range(epochs):
        ...
        for sentence in sentences:
            ...
            sentence = [w for w in sentence
                        # randomly drop words according to p_neg
                        if np.random.random() < (1 - p_drop[w])
                        ]
            if len(sentence) < 2:  # if sentence has only 1 word, it has no meaning
                continue
            ...
            for pos in randomly_ordered_positions:
                ...
                # do one iteration of stochastic gradient descent for both positive and negative labels
...
def get_negative_sampling_distribution(sentences, vocab_size):
    ...
    # normalize it so that this is a valid probability distribution
...
def get_context(pos, sentence, window_size):  # get context and ensure index is in range
    # input:
...
def sgd(input_, targets, label, learning_rate, W, V):
    ...
    # return cost (binary cross entropy) , weight probabilities sometimes end up close to 0 or 1 and break the cost function so we add a small value inside log function to mitigate this
...
def load_model(savedir):
    ...
    npz = np.load('%s/model.npz' % savedir)
    W = npz['arr_0']
    V = npz['arr_1']
    return word2idx, W, V
...
def test_model(word2idx, W, V):
    ...
    for We in (W, (W + V.T) / 2):
        ...
        analogy('good', 'bad', 'awesome', 'terrible', word2idx, idx2word, We)
